...
export const MobileMenu = ({menuOpen, setMenuOpen}) => {
  return (
    <div
        className={`fixed inset-0 z-[60] bg-black/60 backdrop-blur-3xl flex flex-col items-center justify-center transition-opacity duration-700 ease-in-out border-b border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.3),inset_0_1px_2px_rgba(255,255,255,0.1)] transform-gpu will-change-transform
            ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
    >
        <button 
            onClick={()=> setMenuOpen(false)}
            className={`absolute top-6 right-6 sm:top-8 sm:right-8 w-12 h-12 flex items-center justify-center text-white text-3xl focus:outline-none cursor-pointer rounded-full bg-white/5 border border-white/10 transition duration-500 group overflow-hidden hover:scale-105
